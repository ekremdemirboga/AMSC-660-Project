...
lam = 0.0001; % Tikhonov regularization parameter
...
tol = 1e-4;
%
[w,f,gnorm] = stochasticGradient(w,kmax,tol,fun,gfun,bsz);
% [w,f,gnorm] = LevenbergMarquardt(r_and_J,w,kmax,tol);
% [w,f,gnorm] = Nesterov(w,kmax,tol,fun,gfun,bsz);
figure;
plot(f,'Linewidth',2);
...
f = sum(log(1 + exp(-myquadratic2(Xtrain,label,I,w))))/length(I) + 0.5*lam*w'*w;
end
%%
function g = qlossgrad(I,Xtrain,label,w,lam)
aux = exp(-myquadratic2(Xtrain,label,I,w));
a = -aux./(1+aux);
X = Xtrain(I,:);
d = size(X,2);
d2 = d^2;
...
%%
function q = myquadratic2(Xtrain,label,I,w)
X = Xtrain(I,:);
d = size(X,2);
d2 = d^2;
y = label(I);
